# Supplemental Figure 2a: Bulk RNA-seq gene expression boxplots for selected ECM genes (UBC cohort).
library(dplyr)
library(readxl)
library(DESeq2)
library(ggplot2)
...
# Genes to plot
selected_genes <- c("COL10A1", "ASPN", "COMP", "COL1A1", "COL1A2", "COL3A1", "FN1", "FAP", "CCL11", "FCGR3A")
...
ggsave(plot = plt, filename = paste0(supplemental_figure_output_path, "Supplemental_Figure_2a.svg"), width = 1.8 * length(selected_genes), height = 4, units = "cm")
...
write.csv(
  source_data,
  file = paste0(source_data_output_path, "Supplemental_Figure_2a.csv"),
  row.names = FALSE
)
