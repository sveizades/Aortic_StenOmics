# Figure 3e: Scatter plot of FAPI TBRmax versus aortic valve peak velocity with linear regression (SALTIRE3 cohort).
library(ggplot2)
library(dplyr)
library(scales)
...
FAPI_stats <- read.csv("../../05_statistics/03_output/02_FAPI_statistics/Figure_3e_Vmax_TBRmax_correlation.csv")
...
ggsave(
  plot = plt_F303,
  filename = paste0(main_figure_output_path, "Figure_3e.svg"),
  width = 5, height = 4.3, units = "cm"
)
write.csv(FAPI_data[, c("Donor_ID", "AVE_AV_Vmax", "TBRmax")], file = paste0(source_data_output_path, "Figure_3e.csv"), row.names = TRUE)
