# Figure 3f: Scatter plot of FAPI TBRmax versus non-calcific valve thickening volume with linear regression (SALTIRE3 cohort).
library(ggplot2)
library(dplyr)
library(scales)
...
FAPI_data <- FAPI_data[!is.na(FAPI_data$Non_Calcific_Volume), ]
FAPI_stats <- read.csv("../../05_statistics/03_output/02_FAPI_statistics/Figure_3f_NonCalcificThickening_TBRmax_correlation.csv")
...
x_breaks <- c(200, 1000, 2000)
plt_F305 <- ggplot(FAPI_data, aes(x = Non_Calcific_Volume, y = TBRmax)) +
  geom_point(size = 0.3, color = "black", alpha = 1) +
  geom_smooth(method = "lm", se = TRUE, linewidth = 0.4, color = "blue", fill = "blue", alpha = 0.1) +
  annotate("text",
           x = min(FAPI_data$Non_Calcific_Volume, na.rm = TRUE),
           y = max(FAPI_data$TBRmax, na.rm = TRUE),
           label = label_text,
           hjust = 0, vjust = 1,
           size = 1.6, color = "black") +
  scale_x_continuous(labels = label_number(big.mark = ","), expand = expansion(mult = c(0.05, 0.03))) +
...
ggsave(
  plot = plt_F305,
  filename = paste0(main_figure_output_path, "Figure_3f.svg"),
  width = 5, height = 4.3, units = "cm"
)
write.csv(FAPI_data[, c("Donor_ID", "Non_Calcific_Volume", "TBRmax")], file = paste0(source_data_output_path, "Figure_3f.csv"), row.names = TRUE)
