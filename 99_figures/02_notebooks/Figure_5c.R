# Figure 5c: Aortic stenosis haemodynamic progression versus CT calcium progression
...
FAPI_stats <- read.csv("../../05_statistics/03_output/03_FAPI_follow_up/jet_CTcalc_correlation.csv")
...
plt_F503 <- ggplot(follow_up, aes(x = AnnualisedVmaxDifference, y = CTCalciumScore_Annualised_difference)) +
  geom_point(size = 0.3, color = "black", alpha = 1) +
  geom_smooth(method = "lm", se = T, linewidth = 0.4, color = "blue", fill = "blue", alpha = 0.1) +
  annotate("text",
           x = min(follow_up$AnnualisedVmaxDifference, na.rm = TRUE),
...
ggsave(
  plot = plt_F503,
  filename = paste0(main_figure_output_path, "Figure_5c.svg"),
  width = 4, height = 3.8, units = "cm"
)
write.csv(follow_up[, c("Donor_ID", "AnnualisedVmaxDifference", "CTCalciumScore_Annualised_difference")], file = paste0(source_data_output_path, "Figure_5c.csv"), row.names = TRUE)
