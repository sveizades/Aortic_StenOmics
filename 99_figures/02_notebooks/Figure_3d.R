# Figure 3d: Scatter plot of FAPI TBRmax versus CT calcium score with linear regression (SALTIRE3 cohort).
library(ggplot2)
library(dplyr)
library(scales)
...
FAPI_stats <- read.csv("../../05_statistics/03_output/02_FAPI_statistics/Figure_3d_CTcalcium_TBRmax_correlation.csv")
...
x_breaks <- c(0, 10, 30, 100, 300, 1000, 3000, 10000)
plt_F304 <- ggplot(FAPI_data, aes(x = CT_Calcium_Score, y = TBRmax)) +
  geom_point(size = 0.3, color = "black", alpha = 1) +
  geom_smooth(method = "lm", se = TRUE, linewidth = 0.4, color = "blue", fill = "blue", alpha = 0.1) +
  annotate("text",
           x = min(FAPI_data$CT_Calcium_Score, na.rm = TRUE),
...
ggsave(
  plot = plt_F304,
  filename = paste0(main_figure_output_path, "Figure_3d.svg"),
  width = 5, height = 4.3, units = "cm"
)
write.csv(FAPI_data[, c("Donor_ID", "CT_Calcium_Score", "TBRmax")], file = paste0(source_data_output_path, "Figure_3d.csv"), row.names = TRUE)
